...
> **SU-CCB 这套多-agent 协作框架的那块屏幕。** 你用它跑需求、设计、派工、审查，状态却散落在一堆 `docs/` 与 `docs/.ccb/` 文件里 —— SU-Oriel 把这些投影成一个**看得见、点得动**的本地控制台。

SU-Oriel 不是独立工具，它是 [**SU-CCB**](https://github.com/Im-Sue/SU-CCB) 的视图层。它不决策、不执行、不写业务真相 —— 真相始终在项目的文件和数据库里，它只是多挂一块俯瞰全局的屏幕。要看懂它，先得知道它在投影什么。
...
跑过 SU-CCB 的流程你会有一种体感：一个需求从分析、设计、拆分、派工到审查，每一步的状态都落成了文件 —— 节点状态在 `docs/.ccb/state/`，任务散在 `docs/03_开发计划/`，需求在 `docs/02_需求设计/`，扫描和失败留痕埋在 `.ccb` 更深处。它们都在、也都可恢复，但你想知道"现在做到哪了、哪儿卡住了"，得自己翻目录、拼上下文。

SU-Oriel 就是来省掉这个"翻"的。它把散在文件系统里的协作状态一屏归并出来：节点走到哪、任务什么状态、需求和文档长什么样、哪一次扫描或解析失败了 —— 原来要翻五个目录才拼得出的全局，现在俯瞰即得。

它只做两件事，也只该做这两件：**看得见**（把每个工位的状态画在一块屏上）和**点得动**（按按钮触发创建 / 同步 / 重索引 / 重新分析 / 拉起内嵌 slot 终端）。它不写真相 —— 控制台坏了，项目照样靠文件、git 和编辑器跑得通；有它，只是多一双俯瞰全局的眼睛。

> 它为什么这么设计（为什么要审批门、为什么决策与执行分离），理念在 [SU-CCB 根仓](https://github.com/Im-Sue/SU-CCB) 与 [plugin README](https://github.com/Im-Sue/su-ccb-claude-plugin) 里。SU-Oriel 是这套理念**长出来的那块屏幕**。
...
## 三、它在 harness 里的位置：中控大屏，不碰生产

SU-Oriel **不参与生产** —— 不决策、不执行、不写业务真相，它是这条流水线的**中控大屏 / 驾驶舱**。真相源在协议层（kernel）和数据层（docs），SU-Oriel 是**可选的一层视图**：让 harness 的内部状态看得见、点得动，却管不着流水线本身。控制台坏了也不丢数据 —— 没有它，项目照样靠文件、git 和编辑器跑得通。
