...
export type AttentionSeverity = "info" | "warning" | "attention";
...
export async function fetchAttention(projectId: string): Promise<AttentionListResponse> {
  return await requestJson<AttentionListResponse>(`/api/projects/${projectId}/attention`, "加载通知列表失败");
}

export async function ackAttention(projectId: string, ref: string): Promise<AttentionAckResponse> {
  return await requestJson<AttentionAckResponse>(`/api/projects/${projectId}/attention/ack`, "标记通知已读失败", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
...
export async function fetchAttentionSettings(projectId: string): Promise<AttentionSettingsResponse> {
  return await requestJson<AttentionSettingsResponse>(
    `/api/projects/${projectId}/attention/settings`,
    "加载通知设置失败"
  );
}
...
export async function updateAttentionSettings(
  projectId: string,
  input: { dnd_until: string | null }
): Promise<AttentionSettingsResponse> {
  return await requestJson<AttentionSettingsResponse>(
    `/api/projects/${projectId}/attention/settings`,
    "保存通知设置失败",
    {
